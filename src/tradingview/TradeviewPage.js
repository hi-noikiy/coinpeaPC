...
class TradeviewPage extends React.Component {
  ...
  componentDidMount() {
   ...
   /* console.log('window.TradingView ->', window.TradingView);
    ...
    this.websocketStart( this.tradingViewGetReady);
  ...
  // 开启websocket
  websocketStart = (fn) => {
        const wsUrl = window.location.hostname === 'localhost' ? '47.94.194.143' :  window.location.hostname ;
        const wsProtocol = window.location.protocol === 'http:' ?'ws':'wss';
        this.websocketUrl = `${wsProtocol}://${wsUrl}/coinex-interface/kline`;
        window.ws = new WebSocket(this.websocketUrl);
  ...
  tradingViewGetReady = ()  =>{
    ...
    window.TradingView.onready((() => {
     ...
     /*  if(!(window.ws && window.ws.readyState === 1)){  //改
            this.websocketStart();
      } */
  ...
  datafeedConfig = (params) => {
    ...
    let { resolution, Datafeeds, serverUrl, pushInterval, coinPairId, symbolName, pricePre, quanPre , height} = params;
   
    let style = {
          up: "#589065",
          down: "#ae4e54",
          bg: "#1d2b3b",// window.location.pathname.indexOf('/klineTrade') > -1 ?"#1d2b3b":"#181b2a",
    ...
    return {
        ...
        custom_css_url: window.location.pathname.indexOf('/klineTrade') > -1 ?'night2.css':'night2.css',
        //autosize: true,
        ...
        disabled_features: [
            ...
            'header_chart_type', //显示 可设置基准线 美国线啥的
            'header_compare',
            'header_undo_redo',
            'header_screenshot',
            'header_saveload',
        ...
        //  'adaptive_logo',
           ...
           'header_indicators', //图表指标
            ...
            window.location.pathname.indexOf('/klineTrade') > -1 ?'header_fullscreen_button':'' //全屏按钮
        ],
