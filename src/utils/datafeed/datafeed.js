...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
  ...
  if (!this._configuration.supports_group_request ) {
    ...
    /* symbolResolveJSON.name = symbolName;
    symbolResolveJSON.ticker = symbolName; */
    ...
    setTimeout(() => {
        symbolResolveJSON.name= symbolName;
        symbolResolveJSON.ticker = symbolName;
        symbolResolveJSON.full_name = symbolName;
        symbolResolveJSON.base_name = [symbolName];
        symbolResolveJSON.legs = [symbolName];
        symbolResolveJSON.pro_name = symbolName;
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
  ...
  var websocketGetData = function () {
    ...
    let config = {
      ...
      // symbolInfo,
      period,
      relationid: store.getState('trade').trade.coinArea.id,
      from,         //开始时间戳
      to,             //结束时间戳
     // "baseCurrencyId":1,         //基准货币主键
      //"targetCurrencyId": 5         //目标货币主键
    }
    
    console.log('getBars->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', window.hasWsMessage)
    console.log('window.ws.readyState->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', window.ws.readyState)
    
     
   
      
    if (!window.hasWsMessage && window.ws.readyState === 1) {
 ...
 */
  ...
  var dealSuccess = function (res) {
    ...
    let meta = { version: that._protocolVersion, noData: bars.length === 0, nextTime: data.nb || data.nextTime };
...
// 订阅K线数据。图表库将调用onRealtimeCallback方法以更新实时数据。获取所有数据之后， ws第二次发送数据
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function (symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) {

  window.hasWsMessage= false;
  window._onResetCacheNeededCallback = onResetCacheNeededCallback;
  ...
  try {
    for (var symbolIndex = 0; symbolIndex < data.symbol.length; ++symbolIndex) {
      ...
      var symbolInfo = {
        ...
        has_empty_bars: tableField(data, "has-empty-bars", symbolIndex) || true,
